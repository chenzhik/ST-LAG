	...
	def __init__(
		...
		self.transformer_blocks = nn.ModuleList(
			[   # TemporalTransformerBlock
				SwinTemporalTransformerBlock(  # default in animatediff is TemporalTransformerBlock
					dim=inner_dim,
					num_attention_heads=num_attention_heads,
...
###### Author: Zhikai Chen ######
class SwinTemporalTransformerBlock(nn.Module):
	def __init__(
		self,
		dim,
		num_attention_heads,
	...
	def forward(self, hidden_states, pos_encoding=None, encoder_hidden_states=None, attention_mask=None, motion_clips=None):
		
		if hidden_states.shape[1] == 32:
			jitter_range = random.randint(-6, 6) # random.randint(-7, 7)
			motion_clips = [
				(0 , 16), (8 + jitter_range, 24 + jitter_range), (16 , 32)
			]
		elif hidden_states.shape[1] == 48:
			jitter_range = random.randint(-6, 6)
			motion_clips = [
				(0 , 16), (8 + jitter_range, 24 + jitter_range), (16 , 32), 
				(24  + jitter_range, 40  + jitter_range), (32 , 48)
			]
			...
			motion_clips = [
				(0 , 16), (8 + jitter_range, 24 + jitter_range), (16 , 32), 
				(24 + jitter_range, 40 + jitter_range), (32 , 48), 
				(40 + jitter_range, 56 + jitter_range), (48, 64)
			]
		elif hidden_states.shape[1] == 16:
			motion_clips = None
		...
		for attention_block, norm in zip(self.attention_blocks, self.norms):
			...
			if motion_clips is not None:
				...
				for l_start, l_end in motion_clips:
					weight_sequence = generate_weight_sequence(l_end - l_start)
					weight_tensor = torch.ones_like(count[:, l_start:l_end])
					# print(weight_tensor.shape)
					weight_tensor = weight_tensor * torch.Tensor(weight_sequence).to(norm_hidden_states.device).unsqueeze(0).unsqueeze(-1)

					norm_hidden_states_clip = norm_hidden_states[:, l_start:l_end, :]
					hidden_state_clip = attention_block(
						norm_hidden_states_clip,
						encoder_hidden_states=encoder_hidden_states,
						attention_mask=attention_mask,
					) 

					value[:, l_start:l_end, :] += hidden_state_clip * weight_tensor
					count[:, l_start:l_end, :] += weight_tensor

				for fi in range(2):
					norm_hidden_states_dilation = norm_hidden_states[:,fi::2,:]
					hidden_state_dilation = attention_block(
						norm_hidden_states_dilation,
						encoder_hidden_states=encoder_hidden_states,
						attention_mask=attention_mask,
					) 
					value[:, fi::2, :] += hidden_state_dilation
					count[:, fi::2, :] += 1
     
				out_hidden_state = torch.where(count>0, value/count, value)
				hidden_states = out_hidden_state + hidden_states
